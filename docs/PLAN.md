...
· **Active cycle:** v1.3 (virtual pad rig) — PR-2/PR-3/PR-4 all Deck-validated and closed. PR-5 (#151): the quiesce-on-disconnect fix is built, unit/integration-tested (mutation-tested), and **Deck-confirmed closing the original EBUSY race** (0/3 iterations reproduced the issue's exact signature). But Tier 2 validation surfaced a **second, previously-hidden bug**: a FIFO message-loss race in `controller_monitor.sh`/`orchestrator.sh` drops one of two `CONTROLLER_REMOVE` messages when two pads disconnect in the same debounce window, so one slot's `disconnected` flag never flips and its reconnect gets REJECTed outright. Root-cause in progress, 2026-08-02.
