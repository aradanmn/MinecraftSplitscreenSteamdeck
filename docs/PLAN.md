...
· **Active cycle:** v1.3 (virtual pad rig) — PR-2/PR-3/PR-4 all Deck-validated and closed. PR-5 (#151): the quiesce-on-disconnect fix is built, unit/integration-tested (mutation-tested), and **Deck-confirmed closing the original EBUSY race** (0/3 iterations reproduced the issue's exact signature). Tier 2 validation then surfaced a **second, previously-hidden bug** — a FIFO message-loss race in `controller_monitor.sh`/`orchestrator.sh` dropped one of two `CONTROLLER_REMOVE` messages when two pads disconnect in the same debounce window. Root-caused (leading suspect confirmed: no persistent reader fd on the FIFO — `dock_detection.sh` had an existing `# H6: tolerate broken pipe` workaround on the same underlying gap) and **fixed same night**: `_open_fifo_reader` now holds one fd open for the orchestrator's whole lifetime instead of opening/closing per read. Unit/integration-tested (mutation-tested: disabling the fix reproduces 8/9 red). **Not yet Deck-validated** — the Deck went unresponsive (gamescope input wedge, likely from orphaned proxy processes left over by earlier probe runs) partway through this investigation; fix is committed and ready for the next Deck sitting.
...
**Remaining after PR-4: agent ~5h · Deck ~3h** (PR-5 + #70; PR-1's 2h/15min folded into PR-0's cost earlier, PR-2/3/4 actuals now folded in above — PR-4 ran over its original 2h/30min estimate, largely due to the three infrastructure bugs found and fixed live; PR-5's estimate also grew mid-flight after the FIFO message-loss bug surfaced underneath the original fix).
