...
· **Active cycle:** v1.3 (virtual pad rig) — PR-2/PR-3/PR-4/PR-5 all Deck-validated and closed. **PR-5 (#151) CLOSED 2026-08-02**: two bugs found and fixed (the quiesce-on-disconnect race #151 itself describes, plus a second FIFO message-loss bug found underneath it), both Deck-validated together — 3/3 clean iterations, zero reproductions, including the exact symlink-resolution failure that hit every run before the fixes. Issue #151 closed on GitHub. **Next: #70 benchmark pilot** — methodology decided (recommendation from `RESEARCH-136-VIRTUAL-PAD-RIG.md` §5, options 2+3: distance-as-covariate plus log-line-gated phase transitions, deferring work-based scoring), and a pre-flight bearing-scouting plan is written into `tests/benchmark/RUNBOOK.md` — not yet run. See the ladder below.
...
**#70's methodology decision — DECIDED 2026-08-02:** open-loop timed input is
*self-damping under load* — a slower configuration flies less far, generates fewer
chunks, and incurs less load, flattening the very A/B difference being measured. See
`RESEARCH-136-VIRTUAL-PAD-RIG.md` §5. Going with options 2+3 for the first campaign:
record distance travelled as a covariate (makes the confound visible instead of
silent, keeps continuity with the existing v2 dataset) plus gate phase transitions on
log-line events instead of fixed sleeps. Option 1 (score by work done, not wall-clock)
deferred — only worth the bigger protocol change if the covariate shows it matters.
...
2. **v1.3 through PR-5 — done, Deck-validated and closed, 2026-08-02.**
   **#70 (benchmark pilot) is next and last for v1.3** — methodology
   decided (§5 of `RESEARCH-136-VIRTUAL-PAD-RIG.md`, options 2+3), logistics
   worked out and a pre-flight bearing-scouting plan written into
   `tests/benchmark/RUNBOOK.md`, 2026-08-02. **Next Deck sitting: run the
   scouting pass**, then the campaign. Three small items to fold in whenever
   convenient (not blocking, all non-severe): the D3.2 cold-start timeout, the
