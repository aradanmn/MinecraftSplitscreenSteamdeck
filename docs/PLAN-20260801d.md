...
**Last updated:** 2026-08-01 · **Repo:** `aradanmn/MinecraftSplitscreenSteamdeck`
· **Active cycle:** v1.3 (virtual pad rig) — PR-2 opened (#197), not yet Deck-validated
...
| **v1.3** | Virtual pad rig. **In progress.** PR-1 subsumed by PR-0 (#158, already shipped it). PR-2 (rig control surface) opened as #197 — 93/93 CI, 7/7 mutations confirmed red, not yet Deck-validated. |
...
| **PR-2** rig control surface (create/destroy/inject/cleanup, PID-tracked) | **Opened as #197.** `tests/lib/uhid_rig.sh` + `tests/lib/fake_pad.sh` (CI double) + `tests/test_uhid_rig.sh` (93/93). 7/7 planned mutations confirmed red. shellcheck/`bash -n` clean; `test_uhid_pad.sh`/`test_workdir.sh` baselines unchanged. **Not yet Deck-validated** — see PR body §"Not done here" for the exact hardware checks outstanding (real `/dev/uhid` creation, live enumerator queries, burst timing, destroy+recreate, SIGKILL-the-shell). | agent ~2h spent |
| **PR-3** automate `stage3_hotplug` behind `MCSS_VIRTUAL_PADS` (human mode stays default) | Not started. | agent 3h · **Deck 40 min** |
...
2. **v1.3 through PR-3** — the rig starts paying back Deck time the moment
   `stage3_hotplug` automates. This is the next thing to start.
3. **v1.4 last.**
