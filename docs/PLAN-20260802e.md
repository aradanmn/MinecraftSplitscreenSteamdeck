...
· **Active cycle:** v1.3 (virtual pad rig) — PR-2/PR-3 Deck-validated; PR-4 (#71 burst-spawn) repro tooling built 2026-08-02, **not yet Deck-validated** — open question is whether #71 is already fixed by an unrelated architecture change or still needs one
...
| **PR-4** #71 burst-spawn repro + fix | **Repro built AND Deck-validated 2026-08-02 — 3/3 iterations converged, zero reproduction of #71.** `tests/probe-burst-spawn.sh` (new, `tests/probe-*.sh` convention, not wired into `run_all.sh`): burst-creates all 4 rig pads with zero waits between creates (the exact "connect 4 pads, then launch" repro shape), launches docked, asserts quad-geometry convergence per slot with a 150s budget, repeats the cycle since a race is intermittent by nature. **Result: `orchestrator.sh`'s deferred-per-slot-reflow architecture (which post-dates #71's 2026-07-06 filing) does appear to have already fixed it** — 3 separate burst-launches (2 from the first run, 1 clean confirmation run after the teardown fix below) all converged P1-P4 to correct quad positions, no code fix needed. **Not fully closed yet**: the operator directly observed windows the automated geometry check had already passed as not yet visually apparent on screen (twice) — a real render-vs-detected lag under the load of 4 JVMs booting at once, not captured by a position-only check. Extended the post-convergence settle window from 5s to 20s in response; **next Deck sitting should re-run and directly watch P4 to confirm visually**, not just trust the geometry assert, before calling #71 closable. | agent ~4h (spent, incl. two infra bugs found live) · Deck ~50 min (spent) |

**Two real bugs found and fixed live during this validation, both in shared test infrastructure (not just PR-4):**
- **`rig_cleanup()` reentrancy bug** (`tests/lib/uhid_rig.sh`) — its guard variable was set on first call and never reset, so any SECOND call in the same process silently did nothing. Broke the probe's 3rd iteration outright ("pad N is already live"). Fixed: guard resets at the end of a completed pass (still protects against genuine nested re-entry, e.g. a trap firing mid-cleanup — just no longer disables all future calls).
- **`hw_reap_stale_session`'s `kwin_wayland` marker-gate bug** (`tests/hardware/lib/helpers.sh`) — live process inspection proved `kwin_wayland`'s own `/proc/$pid/environ` can never carry the `SPLITSCREEN_DEBUG_LOG=` marker the reap loop checks for (the launcher exports that var to itself mid-execution, which only affects children forked afterward; `kwin_wayland` is exec'd via a wrapper in a way that doesn't inherit it either). This left orphaned `kwin_wayland` processes — and a black screen with Steam stuck thinking the game was still running — after **three separate teardowns this session** (the same shape as the original PR-3 incident, which this almost certainly also explains, retroactively). Fixed: `kwin_wayland` is now killed unconditionally in `hw_reap_stale_session`, matching how `reaper`/`bwrap.*PolyMC`/`latestUpdate` were already handled — safe on this platform since `kwin_wayland` only ever runs as this project's own nested session. Confirmed fixed by the clean 3rd-iteration run: zero orphans, `get_display_mode` read back `docked` immediately, operator confirmed Game Mode UI up normally — no manual Steam restart needed, unlike every prior teardown this session.
...
1. **v1.2.4 shipped — done.** #122, #184/#185/#186, #126 all closed and
   ...
   in backlog.
2. **v1.3 through PR-3 — done, Deck-validated 2026-08-02.** PR-4 (#71
   burst-spawn) repro tooling built same day, **needs its first Deck run**
   (`bash tests/probe-burst-spawn.sh`) — the outcome decides whether PR-4
   also needs a code fix or is just a confirmation-and-close. Three small
   items to fold in whenever convenient (not blocking, all non-severe): the
   rig-cleanup pidfile-orphan bug, the D3.2 cold-start timeout, and the
   destroy-latency `dmesg` remeasurement from PR-2 — bundle whichever fit
   into the same Deck sitting as the PR-4 run.
3. **v1.4 last.**
