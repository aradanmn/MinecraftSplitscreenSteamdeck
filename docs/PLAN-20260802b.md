...
· **Active cycle:** v1.3 (virtual pad rig) — PR-2 (#197) Deck-validated; PR-3 (stage3_hotplug automation) **Deck-validated 2026-08-02**, 33 passed/2 failed/17 skipped, two follow-ups filed (rig-cleanup pidfile bug, D3.2 cold-start timeout)
...
| **v1.3** | Virtual pad rig. **In progress.** PR-1 subsumed by PR-0 (#158, already shipped it). PR-2 (rig control surface) opened as #197 — 93/93 CI, 7/7 mutations confirmed red, **Deck-validated 2026-08-01**: core lifecycle confirmed on real hardware, one earlier "burst enumeration broken" scare fully explained (MAX_PLAYERS cap contention, not a bug), one destroy-latency question left genuinely open pending a `dmesg`-based remeasurement. PR-3 (`stage3_hotplug` automation behind `MCSS_VIRTUAL_PADS`) **built and Deck-validated 2026-08-02** — 33 passed/2 failed/17 skipped on real hardware; failures plausibly cold-start timing, not a regression; one real rig-cleanup pidfile bug found (no process leak) and one operational lesson (session-stop needs a manual Steam-overlay Exit Game), both filed as follow-ups, neither blocking. |
...
1. **v1.2.4 shipped — done.** #122, #184/#185/#186, #126 all closed and
   ...
   in backlog.
2. **v1.3 through PR-3 — done, Deck-validated 2026-08-02.** PR-4 (#71 burst
   spawn) is next. Two small follow-ups to fold in whenever convenient (not
   blocking, both non-severe): the rig-cleanup pidfile-orphan bug, and the
   D3.2 cold-start timeout. The destroy-latency `dmesg` remeasurement from
   PR-2 (see `docs/RESEARCH-136-VIRTUAL-PAD-RIG.md`/memory) is still open too
   — bundle it into the next Deck sitting alongside whichever of these gets
   picked up.
3. **v1.4 last.**
...
## Deck state (2026-08-02, evening — session stopped, Deck otherwise idle)

- **On `feat/136-pr2-rig-surface` @ `df43b86`**, deployed and `deploy.sh
  --check`-verified fresh immediately before the PR-3 validation run.
