...
| **PR-4** #71 burst-spawn repro + fix | **Repro tooling built, agent-side, 2026-08-02 — not yet Deck-validated.** `tests/probe-burst-spawn.sh` (new, matches the `tests/probe-*.sh` one-off-bug-repro convention — NOT wired into `run_all.sh`): burst-creates all 4 rig pads with zero waits between creates (`tests/lib/uhid_rig.sh`, the exact "connect 4 pads, then launch" shape from #71's repro), launches docked, and asserts quad-geometry convergence per slot with a 150s budget (spawn_instance's own window poll allows up to 120s for a cold JVM). Repeats the full stop/burst-launch/assert cycle (default 5x, `$1` or `MCSS_BURST_ITERATIONS` override) since a race is intermittent by nature — reports a per-iteration verdict plus a final "N/5 converged" tally, not a single pass/fail. Between iterations it stops the session (`hw_stop_orchestrator`) and checks `get_display_mode` read back `docked`; if not, it pauses with an on-screen recovery prompt rather than silently relaunching into the stale-gamescope-state PR-3 found (PRINCIPLES #7 — no remote Steam/gamescope touches). **Open question this probe exists to answer, not yet run**: since #71 was filed (2026-07-06), `orchestrator.sh`'s per-slot spawn path changed to defer `_reflow_layout` until after that slot's own window exists — on paper this should already fix the race (the straggler's own late reflow recomputes the whole layout), but nobody has confirmed it against the original repro shape (`docs/RUNBOOK-HW2-TEST-HARNESS.md`'s Part 4 marked it optional and was never run). No fix has been written — writing one before confirming the bug still exists would be pure speculation (PRINCIPLES #3/#4, same discipline as PR-3). **Next: run it on the Deck** (`bash tests/probe-burst-spawn.sh`) to find out whether #71 is already fixed or still needs one. | agent ~2h (spent) · Deck 30 min (still owed) |
